...
# 可以通过 ls 查看script下是否多了supervisord.conf这个文件，表示初始化配置生成了。
...
创建`fsapi_celery_worker.ini`文件，启动我们项目worker主进程

```bash
cd fspi/script
touch fsapi_celery_worker.ini
```
...
创建`fsapi_celery_beat.ini`文件，来触发我们的beat定时计划任务

```bash
cd fsapi/script
touch fsapi_celery_beat.ini
```
...
创建`fsapi_celery_flower.ini`文件，来启动我们的celery监控管理工具

```bash
cd fsapi/script
touch fsapi_celery_flower.ini
```
...
[Service]
Type=forking
ExecStart=/home/hkw/anaconda3/envs/fsapi/bin/supervisord -n -c /home/hkw/Desktop/fsapi/fsapi/scripts/supervisord.conf
ExecStop=/home/hkw/anaconda3/envs/fsapi/bin/supervisorctl $OPTIONS shutdown
ExecReload=/home/hkw/anaconda3/envs/fsapi/bin/supervisorctl $OPTIONS reload
KillMode=process
Restart=on-failure
RestartSec=42s
