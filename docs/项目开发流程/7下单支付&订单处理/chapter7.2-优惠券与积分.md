...
![image-20211129085657537](https://img2023.cnblogs.com/blog/2570053/202304/2570053-20230406192132510-1498428924.png)
...
-- 优惠券测试数据
truncate table fs_coupon;
INSERT INTO fs_coupon (id, name, is_deleted, orders, is_show, created_time, updated_time, discount, coupon_type, total, has_total, start_time, end_time, get_type, `condition`, per_limit, sale) VALUES (1, '30元通用优惠券', 0, 1, 1, '2022-05-04 10:35:40.569417', '2023-06-30 10:25:00.353212', 1, 0, 10000, 10000, '2022-05-04 10:35:00', '2023-11-02 10:35:00', 0, 100, 1, '-30'),(2, '前端学习通用优惠券', 0, 1, 1, '2022-05-04 10:36:58.401527', '2024-05-04 10:36:58.401556', 1, 1, 100, 100, '2022-05-04 10:36:00', '2024-08-04 10:36:00', 0, 0, 1, '-50'),(3, 'Typescript课程专用券', 0, 1, 1, '2022-05-04 10:38:36.134581', '2024-05-04 10:38:36.134624', 2, 3, 1000, 1000, '2022-05-04 10:38:00', '2024-08-04 10:38:00', 0, 0, 1, '*0.88'),(4, 'python七夕专用券', 0, 1, 1, '2022-05-04 10:40:08.022904', '2024-06-30 10:25:46.949197', 1, 2, 200, 200, '2022-05-04 10:39:00', '2024-11-15 10:39:00', 1, 0, 1, '-99'),(5, '算法学习优惠券', 0, 1, 1, '2021-08-05 10:05:07.837008', '2024-06-30 10:26:12.133812', 2, 2, 1000, 1000, '2022-08-05 10:04:00', '2024-12-25 10:04:00', 0, 200, 1, '*0.85');

-- 优惠券与学习方向的关系测试数据
truncate table fs_coupon_course_direction;
INSERT INTO fs_coupon_course_direction (id, created_time, coupon_id, direction_id) VALUES (1, '2024-05-04 10:36:58.414461', 2, 1);

-- 优惠券与课程分类的关系测试数据
truncate table fs_coupon_course_category;
INSERT INTO fs_coupon_course_category (id, created_time, category_id, coupon_id) VALUES (1, '2023-05-04 10:40:08.029505', 20, 4),(2, '2024-05-04 10:40:08.042891', 21, 4),(3, '2024-08-05 10:05:07.966221', 33, 5);

-- 优惠券与课程信息的关系测试数据
truncate table fs_coupon_course;
INSERT INTO fs_coupon_course (id, created_time, coupon_id, course_id) VALUES (1, '2024-05-04 10:38:36.140929', 3, 1),(2, '2024-05-04 10:38:36.143166', 3, 2);

-- 优惠券的发放和使用日志的测试数据
truncate table fs_coupon_log;
INSERT INTO fs_coupon_log (id, is_deleted, orders, is_show, created_time, updated_time, name, use_time, use_status, coupon_id, order_id, user_id) VALUES (5, 0, 1, 1, '2022-05-04 12:00:25.051976', '2024-06-30 10:25:17.681298', '30元通用优惠券222', null, 0, 1, null, 1),(8, 0, 1, 1, '2022-05-04 12:03:24.331024', '2024-06-30 10:22:45.834401', '前端学习通用优惠券', null, 0, 2, null, 1),(9, 0, 1, 1, '2022-05-04 12:03:31.692397', '2024-06-30 10:23:41.492205', 'Typescript课程专用券', null, 0, 3, null, 1),(10, 0, 1, 1, '2022-05-04 12:03:38.225438', '2024-06-30 10:25:49.797318', 'python七夕专用券', null, 0, 4, null, 1),(11, 0, 1, 1, '2022-05-04 12:09:25.406437', '2024-06-30 10:23:55.832262', '前端学习通用优惠券', null, 0, 2, null, 1),(12, 0, 1, 1, '2021-08-05 10:06:06.036230', '2024-06-30 10:26:20.723668', '算法学习优惠券', null, 0, 5, null, 1);
...
```bash
feature: 实现后台管理员给用户分发优惠券时自动记录到redis中
```
...
```bash
feature: 服务端实现获取用户所有优惠券与本次下单的可用优惠券列表
```
...
```bash
feature: 客户端展示用户本次下单的可用优惠券并重新调整价格
```
...
```bash
feature: 服务端在用户选择优惠券以后重新计算订单实付价格
```

`orders/seriazliers.py`，订单创建序列化类解耦合
...
class OrderModelSerializer(serializers.ModelSerializer):
    ...
    def get_validated_data(self, validated_data):
        ...
        return user_id, pay_type, user_coupon_id, user_coupon
    ...
    def discount_credit(self, cart_hash, order, user_coupon):
        # 提取购物车中所有勾选状态为b'1'的商品
        course_id_list = [int(key.decode()) for key, value in cart_hash.items() if value == b'1']
    ...
    def create(self, validated_data):
        ...
        redis = get_redis_connection("cart")
        # 获取数据
        user_id, pay_type, user_coupon_id, user_coupon = self.get_validated_data(validated_data)
        # 开始事务
        with transaction.atomic():
            # 设置事务回滚的标记点,一个事物中可以设置多个回滚标记
            transaction_start = transaction.savepoint()
            try:
                ...
                # 订单折扣和积分优惠
                self.discount_credit(cart_hash, order, user_coupon)
                # todo 支付链接地址[后面实现支付功能的时候，再做]
                order.pay_link = ""
                # 删除购物车中被勾选的商品，保留没有被勾选的商品信息
...
我们之前在自定义用户模型的时候，已经声明了积分字段，所以此处为了方便后面开发积分功能的时候，能够在admin管理站点中进行积分的调整使用，所以我们此处在user/admin.py后台站点配置文件中，配置user用户模型的模型管理器。
...
课程模型新增积分字段，`course/models.py`，代码：
...
接下来，在课程详情展示页面中新增显示当前课程可以抵扣的积分数量。`course/serializers.py`，代码：
...
`apps/course/search_indexes.py`，代码：
...
~~~python
feature: 服务端user/models新增积分模型表，其他有关模型表新增积分字段，admin后台新增积分字段，新建订单序列化类优化解耦合
~~~
